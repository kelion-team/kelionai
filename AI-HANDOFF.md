...
- ✅ **Release avatar v2.3 APROBAT de Adrian (12 iul 2026):** branch `avatar-si-alerte` conține commit `0a35711` (expresii faciale ARKit, gesturi comandate, cameră low-light, error reporting client, LiveKit self-hosted, deploy anti-fantomă) plus reparația cameră de mai sus. Branch-ul a fost adus la zi cu `origin/master`, s-a deschis PR prin `kelion-github pr` și s-a făcut **merge în `master`**. **Deploy-ul NU a fost executat în această sesiune (`kelion-github deploy` neporbit), la cererea explicită a utilizatorului — urmează să fie declanșat separat de Adrian.**
