...
- **Componente:** `ChatPanel.tsx` (panoul de chat), `AdminPanel.tsx` („Vezi chat" per user, „Tradu în română", Inbox live, finanțe, useri, tab **„Echipă"** — vezi mai jos), `CustomerSettings.tsx` (⚙ client: Preferințe, Credit cu mențiunea 25%, BYOK, Cont+ștergere), `ContactModal.tsx`, `WalletButton.tsx` (credit + reminder escaladant), `AvatarModel.tsx` (randare RPM + idle + lip-sync moderat intenționat), `AvatarLoading.tsx` (progres GLB ~9MB, altfel ecran negru pe mobil), `CameraView.tsx` (ochii lui Kelion — ascuns pe ecran, gard anti-cadru-negru), `CardView.tsx` (rezultate skill: email/calendar/tasks/Drive/contacts/search), `VisitorChatWidget.tsx` (chat live vizitator↔OWNER, nu AI, poll 3s).

**Canalul de echipă (Adrian, 11 iul: „buton chat în care suntem toți 3 conectați").** Discuție LIVE persistentă Adrian + Kelion + Claude, gândită extensibil (`author` e text liber, nu enum — orice AI nou de la orice firmă se alătură fără schimbare de schemă). Tabel `team_channel` (§9), distinct de `shared_memory` (caiet = jurnal de lucru append-only): asta e chat cu adresare clară (`addressed_to`).
- Rute admin (sesiune, cookie): `GET/POST /api/admin/team-channel` — tab „Echipă" în `AdminPanel.tsx`, poll 4s, selector destinatar.
- Rută prin `BRIDGE_SECRET` (comună — **nu** distinge apelantul): `GET/POST /api/bridge/team-channel` — folosită de Claude (fără cookie de sesiune, se trezește prin `team-channel.yml`) ȘI de Kelion direct de pe VPS (are același secret în `bridge-secret.txt`). POST acceptă `author` opțional în body (implicit `"claude"`) ca Kelion să se poată identifica corect quando postează direct.
- Adresat către **Kelion** → forward automat pe puntea reală (`forwardToKelion()` în `bridge.ts`, folosit simetric de AMBELE rute POST — admin ȘI bridge), răspunsul se postează singur înapoi în canal.
- **BUG găsit+reparat 11 iul (PR #89):** GET-ul implicit (fără `after`) întorcea mereu `{"messages":[]}` deși rândurile chiar existau în DB (dovadă: `?after=1` le întorcea corect). `getTeamChannel()` în `db.ts` rescrisă simplu (`ORDER BY id DESC LIMIT $1` + `.reverse()` în JS, fără subquery cu parametru nefolosit) — verificat live după deploy, GET-ul implicit întoarce acum firul complet.
...
**A TREIA LECȚIE — repor­niri Railway NEEXPLICATE, în afara pipeline-ului (11 iul, GĂSITĂ, NECAUZĂ-IDENTIFICATĂ).** În timp ce diagnosticam bug-ul de mai sus, `/api/version` a arătat 3 boot-uri diferite într-un interval de ~24 min (08:46:57 → 08:59:38 → 09:09:56) deși `deploy.yml` (singurul care publică prin pipeline-ul verificat) a rulat o SINGURĂ dată în tot intervalul ăsta (pentru merge-ul PR #88, la 08:45:58). Deci ceva a repornit serviciul Railway de cel puțin 2 ori PE CONT PROPRIU, în afara CI-ului — probabil `railway up` rulat manual (de pe VPS, de Kelion sau de un script al lui), posibil altceva (Railway însuși — OOM/crash). **Nu s-a identificat cauza exactă** — doar dovada că se întâmplă. Riscul: dacă cineva rulează `railway up` cu cod care nu e sincronizat 100% cu `origin/master`, se încalcă regula §1 „producție = master" fără ca `deploy.yml` să prindă asta (nu există alertă pentru deploy-uri din afara pipeline-ului). **De investigat de oricine preia asta:** verifică `railway-deploy-status.yml` (§7) pentru istoricul REAL de deploy-uri Railway (nu doar cele din GitHub Actions) și, dacă Kelion are un script de auto-deploy pe VPS, verifică dacă rulează `railway up` fără să verifice mai întâi că VPS-ul e sincronizat la `origin/master`.
...
- `read-caiet.yml` — diagnostic doar-citire: `shared_memory` (caiet) + istoricul admin, prin `BRIDGE_SECRET`. Autorizat explicit de Adrian (conținutul apare în jurnalul rulării — repo privat).
- `team-channel.yml` — citește/postează în canalul de echipă (§2) prin `BRIDGE_SECRET`; input opțional `post_content`/`post_to` pentru postare, `after` pentru diagnostic (ocolește implicitul).
...
- ✅ Live și verificat: setări client (⚙), căutare web max, router model, memorie dublată + căutare full-text reală (LIVE, nu „next"), ora Londrei, QR Linux → `/dl/Kelionai-linux.zip`, deploy-uri anti-fantomă (repo), fixul „calea rulată" (§6, confirmat cu md5 diferit înainte/după + marker `warmPub` pe disc), **canalul de echipă Adrian+Kelion+Claude** (§2 — live, testat de-adevăratelea: mesaje reale schimbate în ambele direcții, cu forward automat funcțional către Kelion).
...
- 🔴 **DE INVESTIGAT — repor­niri Railway neexplicate, în afara pipeline-ului CI.** Vezi §6 „A TREIA LECȚIE". Nu blochează nimic funcțional acum, dar e o gaură în garanția „producție = master" dacă cineva rulează `railway up` cu cod nesincronizat.
...
- Istoric PR-uri relevante: #72 (fantomă+setări), #73 (router+memorie+stall), #74 (secret VPS+zombie), #75 (standby), #76 (QR Linux), #77 (sesiuni calde per-vizitator + prima bucată instant + ora Londrei), #78/#79 (acest document), #80 (fix cale rulată systemd — CONFIRMAT FUNCȚIONAL), #81 (memorie full-text), #82 (acest document, runda 2), #84 (diagnostic caiet+istoric, autorizat explicit), #85 (canalul de echipă — tabel + rute admin), #86 (rută bridge pentru Kelion), #87 (workflow team-channel), #88 (forward simetric Claude→Kelion), #89 (fix GET gol + mislabel author — testat live, dovadă în §6/§2).
