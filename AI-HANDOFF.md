...
- Milestone-uri nepornite: sandbox de cod per user obișnuit (nu doar admin), aplicații native „full packaged” (iOS App Store etc.), Picovoice wake word, LiveKit full-duplex (cheile există „în backup”, neconfigurate), carduri rezultate skill-uri (parțial — `CardView.tsx` există), monetizare completă (Stripe 75/25 există pe webhook), panou admin extins, curățare cod mort (`turnManager.ts` + orice altceva găsit de un audit nou), memorie cu embeddings/AI reale (dincolo de full-text), profilare fină a latenței (§4.4).
