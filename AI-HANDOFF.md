...
- **Componente:** `ChatPanel.tsx` (panoul de chat), `AdminPanel.tsx` („Vezi chat" per user, „Tradu în română", Inbox live, finanțe, useri), `CustomerSettings.tsx` (⚙ client: Preferințe, Credit cu mențiunea 25%, BYOK, Cont+ștergere), `ContactModal.tsx`, `WalletButton.tsx` (credit + reminder escaladant), `AvatarModel.tsx` (randare RPM + idle natural de la gât în jos — respirație vizibilă, umeri, trunchi, balans de greutate, gesturi ocazionale subtile ale mâinilor + **gesturi comandate de creier: `raiseRightHand`, `salute`, `pointMonitor`** + **expresii faciale naturale stil gentleman** folosind blendshape-urile ARKit: clipit variabil, zâmbet, sprâncene, mirare, gândire, micro-expresii context/ton, fără a afecta respirația/lip-sync), `AvatarLoading.tsx` (progres GLB ~9MB, altfel ecran negru pe mobil), `CameraView.tsx` (ochii lui Kelion — ascuns pe ecran, gard anti-cadru-negru, **boost low-light pe canvas**, **serializare + retry la pornirea camerei pentru erori `AbortError`/`NotReadableError` pe mobil**), `CardView.tsx` (rezultate skill: email/calendar/tasks/Drive/contacts/search), `VisitorChatWidget.tsx` (chat live vizitator↔OWNER, nu AI, poll 3s).
- **Lib (`src/lib/`):** `voice.ts`/`audioIO.ts` (voce — anti-ecou `VOICE_GAP_MS=1800` NU se atinge; **plus extragere features vocale client-side**: F0, centroid spectral, rolloff, ZCR, RMS, jitter, shimmer pentru identificare speaker/gen), `updateCheck.ts` (sursă unică versiune: filigran + sub QR, ora Londrei), `chat.ts` (streaming client, „ceas de gardă" 50s + `/api/chat/resume`), `admin.ts` (toate fetch-urile panoului admin), `api.ts` (auth Google + `startGoogleConnect` consimțământ incremental + `startDemo`), `camera.ts` (**low-light constraints**: expunere/gain continue + boost EV/ISO; **acces serializat la cameră cu release delay + retry tranzitorii + AbortSignal**), `errorReporting.ts` (colectare erori consolă frontend → `/api/bridge/client-errors`), `fingerprint.ts` (anti-abuz demo, SHA-256 semnale browser), `i18n.ts` (7 limbi UI), `micStream.ts` (dictare live, pauză 3000ms închide fraza — „ordinul lui Adrian"; colectează features vocale în paralel pentru identificare), `recorder.ts` (clipuri promo admin), `turnManager.ts` (**COD MORT — neimportat**, vezi §10), `utteranceCoalescer.ts` (leagă fragmente STT, debounce 900ms), `wakelock.ts`, `workspace.ts` (mod monitor multi-taburi, o singură voce), `prefs.ts`, `billing.ts`, `languages.ts`.
...
- ✅ **Reparat eroarea perpetuum „camera nu pornește: AbortError (facing=user)" (12 iul 2026):** cauza rădăcină era accesul neserializat la cameră și lipsa unui delay de eliberare a senzorului între stop/start, ceea ce pe mobil genera `AbortError`/`NotReadableError` când `getUserMedia` era apelat înainte ca hardware-ul să elibereze camera anterioară (inclusiv sub React StrictMode / remount rapid). Rescris `frontend/src/lib/camera.ts`: serializare globală a operațiilor de start/stop printr-un lanț de release, delay de 450 ms după `track.stop()`, retry cu backoff (3 încercări, 300/700/1200 ms) pentru erori tranzitorii (`AbortError`, `NotReadableError`, `TrackStartError`), clasificare fatal vs. tranzitoriu, suport pentru `AbortSignal` și expunerea helperelor `isFatalCameraError`/`getCameraErrorCode`/`requestCameraRelease`. Actualizat `frontend/src/components/CameraView.tsx`: folosește `AbortController` pentru cleanup curat, ignoră erorile provocate de propriul abort, raportează în consolă doar erorile reale cu cod și `facing`, și reîncearcă automat la `focus`/`visibilitychange`/`online` când pagina revine. Build backend + frontend + teste trec. Schimbările au fost comise pe branchul `avatar-si-alerte` și pregătite pentru PR/merge în master.
- ✅ **Release avatar v2.3 APROBAT de Adrian (12 iul 2026):** branch `avatar-si-alerte` conține commit `0a35711` (expresii faciale ARKit, gesturi comandate, cameră low-light, error reporting client, LiveKit self-hosted, deploy anti-fantomă) plus reparația cameră de mai sus. Branch-ul a fost adus la zi cu `origin/master`; `kelion-github pr` a returnat `403 Resource not accessible by personal access token` (tokenul permite push dar nu deschiderea PR-urilor), deci s-a făcut **merge fast-forward direct în `master`** pentru a respecta regula „producția = master, 100% în sync". **Deploy-ul NU a fost executat în această sesiune (`kelion-github deploy` neporbit), la cererea explicită a utilizatorului — urmează să fie declanșat separat de Adrian.**
