...
- ✅ **Scara avatarului persistă pentru Adrian (12 iul 2026, cerința lui Adrian: „trebuie sa salvezi marimea avatarului actuala sa nu mai renina mica"):** adăugat `frontend/src/lib/avatarScale.ts` cu `useAvatarScale()` — citește/scrie `kelion-avatar-scale` în localStorage (default 1.65) și sincronizează componentele printr-un event custom. `frontend/src/components/AvatarModel.tsx` folosește acum scala persistată în loc de `scale={1.65}` hard-codat. În `frontend/src/pages/Stage.tsx`, modul de aranjare (dublu-click + rotiță) ajustează scala REALĂ a modelului 3D, nu doar scala containerului; la încărcare, o veche scală de container (`avatar-box.s ≠ 0.42`) este migrată automat în `kelion-avatar-scale` pentru a păstra mărimea vizibilă setată de Adrian. Build backend + frontend trece. Deploy NU a fost executat (la cererea lui Adrian).
