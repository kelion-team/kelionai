...
- ✅ **Release avatar v2.3 APROBAT de Adrian (12 iul 2026):** branch `avatar-si-alerte` conține commit `0a35711` (expresii faciale ARKit, gesturi comandate, cameră low-light, error reporting client, LiveKit self-hosted, deploy anti-fantomă). Branch-ul este împins pe `origin/avatar-si-alerte`, adus la zi cu `origin/master` (25 de commituri înainte) și pregătit pentru PR → merge în `master` → deploy prin `kelion-github deploy` / `deploy.yml` (NU `railway up` direct); deploy-ul NU a fost executat în această sesiune, la cererea explicită a lui Adrian.
