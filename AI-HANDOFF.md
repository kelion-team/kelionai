...
**Neconfigurate încă (din STATUS.md, verifică dacă s-au adăugat între timp):** `VITE_GOOGLE_MAPS_KEY` (există „în backup"), **Picovoice** (nedisponibil — wake word merge pe Web Speech interimar).
**LiveKit (NOU, self-hosted pe VPS):** `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET` — se generează automat de `bridge/kelion-livekit` pe VPS și se scriu în Railway prin GraphQL API (fără acțiune manuală). Verificare: `GET /api/admin/livekit/status` și `POST /api/admin/livekit/token` (admin-only).
...
## 13. STAREA LA 12 IULIE 2026 + CE URMEAZĂ
