...
- ✅ Live și verificat: setări client (⚙), căutare web max, router model, memorie dublată + căutare full-text reală (LIVE, nu „next"), ora Londrei, QR Linux → `/dl/Kelionai-linux.zip`, deploy-uri anti-fantomă (repo), fixul „calea rulată" (§6).
- 🔴 **#7 latență — ÎNCĂ ÎN AȘTEPTARE, altă rulare `bridge-deploy` necesară.** Adrian A RULAT `bridge-deploy` o dată (06:39, 11 iul) — a sincronizat secretul + repo-ul, dar codul worker-ului tot n-a ajuns pe VPS din cauza bugului „calea rulată ≠ calea din repo" (§6), descoperit ABIA după acea rulare (măsurătoare live: tot 13s stall + eroare). Fix-ul e acum în master (commit după #80) — **trebuie o rulare NOUĂ** ca să se aplice cu adevărat. După ea: `public-latency-test` + verifică jurnalul arată „MISMATCH găsit — copiat codul corect" și `grep warmPub` confirmat pe disc.
- Milestone-uri nepornite: sandbox de cod per user obișnuit (nu doar admin), aplicații native „full packaged" (iOS App Store etc.), Picovoice wake word, LiveKit full-duplex (chei „în backup", neconfigurate), carduri rezultate skill-uri (parțial — `CardView.tsx` există), monetizare completă (Stripe 75/25 există pe webhook), panou admin extins, curățare cod mort (`turnManager.ts` + orice altceva găsit de un audit nou), memorie cu embeddings/AI reale (dincolo de full-text).
- Istoric PR-uri relevante: #72 (fantomă+setări), #73 (router+memorie+stall), #74 (secret VPS+zombie), #75 (standby), #76 (QR Linux), #77 (sesiuni calde per-vizitator + prima bucată instant + ora Londrei), #78/#79 (acest document), #80 (fix cale rulată systemd), #81 (memorie full-text).
