...
describe('contractele runtime ale Constructorului în AdminPanel', () => {
  ...
  it('ține Gestures single-flight și derivă displayul Constructor fail-closed', () => {
    const source = fs.readFileSync(new URL('./components/AdminPanel.tsx', import.meta.url), 'utf8')
    expect(source).toContain('gestSavePendingRef.current')
    expect(source).toContain('saveDisabledGesturesCanonical(next)')
    expect(source).toContain('setGestOff(await fetchDisabledGestures())')
    expect(source).toContain('disabled={!gestOffData || gestSaving}')
    expect(source).not.toContain('const inainte = gestOff')
    expect(source).toContain('constructorWorkerCanStartNow === true')
    expect(source).toContain('constructorAcceptingWork === true')
    expect(source).not.toContain("constructorId.state === 'ready'")
    expect(source).not.toContain("constructorId.state === 'busy'")
    expect(source).toContain('parseAdminBuildArchive')
    expect(source).toContain('parseAdminArchiveAcknowledgement')
  })
})
