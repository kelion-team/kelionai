...
// AUDIO I/O — DOAR REDAREA (21 aug clean-slate: „scoate TOT codul de voce mort").
// Jumătatea de CAPTURĂ (microfon + extractor de amprentă) — startMic (cu uploaderul
// `fetch('/api/asr')`), calibrateVoiceprint, hasVoiceprint/loadVoiceprint,
// buildVoiceFeatures/finalizeVoiceFeatures, estimateF0/Centroid/Zcr/Energy/Rolloff,
// tipurile MicHandle/VoicePrint/VoiceFeatures/VoiceFeatureMeta și constantele
// VOX/VOICEPRINT — a fost ȘTEARSĂ: nu mai avea niciun apelant viu (calea vocală s-a
// scos, ChatPanel importă doar redarea). Amprenta vocală rămâne LIVE pe altă cale
// (CustomerSettings.onRecordVoiceprint + routes/voiceprint.ts), care își calculează
// singură features-urile.
//
// Ce a rămas AICI, intact: REDAREA. Vocea creierului vine gata sintetizată de server
// (/api/tts → narator) ca MP3 base64 și aici e DOAR decodată + redată; bucla de
// analiză a nivelului (RMS) mișcă gura avatarului (narator/apel o folosesc).
//  • playVoice(base64) — decodează MP3-ul și îl redă; pe mobil cade sigur pe
//    AudioContext-ul deblocat pe gest (redare deterministă a sunetului întârziat).
//  • getVoiceLevel() — nivelul (0..1) al vocii redate acum, pentru lip-sync.
