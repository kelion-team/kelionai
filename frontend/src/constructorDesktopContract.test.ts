...
describe('Kelion Constructor desktop contract', () => {
  it('reuses the canonical Kelion queue UI and authenticated API', () => {
    ...
    expect(constructorRoute).toContain("exactKeys(req.body, ['profile', 'doctorCapability'])")
    expect(constructorRoute).toContain('claimNextBuildJob(taskId, measuredProfile, req.body.doctorCapability)')
