...
| L1m | AGENȚII ENTERPRISE „pentru tot, inclusiv skilluri" (Adrian, 3 aug seara) + conectorii „la maxim": ownerul a aprins API-urile și a dat rolul → `engines.list` 200 peste tot; lanțul de creare smuls pas cu pas din erori măsurate (enum `GENERIC`; data store obligatoriu — creat ✓ `kelion-cunostinte`; tipul CHAT = fundătură Dialogflow 403 → trecut pe motor SEARCH). Scriptul `scripts/creaza-agenti-enterprise.mjs` ține tot lanțul + rosterul (~32 agenți); rularea finală cu motor SEARCH era ÎN CURS la scrierea acestui rând — dovada = lista re-citită din API, nu acest rând. Conectorii third-party cer consimțământ OAuth uman (o dată/serviciu); Workspace first-party se leagă din API după ce motorul există | aproape — ultima rulare în curs; NEVERIFICAT până nu iese lista din API |
