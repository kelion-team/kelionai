...
| K8 | De unde apar cuvinte ca „Greț" — n-am scris nimic | 🔧 | Intrare-fantomă în chat (bulă user „Greț." fără ca tu să scrii). Cel mai probabil transcrierea vocală (STT) inventează cuvinte din tăcere/zgomot. De reparat: prag de energie + să nu trimită transcrieri fără vorbire reală. | REZOLVAT (5 AUG — filtru `isPhantomTranscript` + verificarea energiei RMS `isSpeechAudio` în STT, test `asrPhantomFilter.test.ts`) |
