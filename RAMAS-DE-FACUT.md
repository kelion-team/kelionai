...
| C6 | **„Răspuns = nimic" — chatul se termina în TĂCERE** | ✅ **gata & verificat** (PR #581): reparat cu 3 plase (`areCevaDeVazut`, reîncercare model rezervă, mesaj + `[CHAT MUT]` în jurnal). 7 teste în `turaMuta.test.ts`. | **verificat**: tratarea răspunsului vid funcționează corect, plasele de protecție previn tura mută și informează utilizatorul. |
