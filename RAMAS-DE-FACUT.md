...
| D5 | **Barge-in prin STT streaming** | ✅ **REZOLVAT (21 feb 2025)**: VAD/Barge-in local în `micStream.ts` când microfonul este mut (cât timp Kelion vorbește/redă TTS). Detectează nivelul RMS peste prag susținut după garda de onset (anti-ecou local) și apelează `onBargeIn` + `onSpeechBegin`. Testat unitar în `backend/src/bargeIn.test.ts`. |
