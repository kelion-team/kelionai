...
| K15 | „ceva toacă creditul OpenRouter, caută soluții, că e faliment curat" | ✅ | RESOLVAT: OpenRouter a fost extirpat complet pe 3 aug (0% consum OpenRouter), tot sistemul trece prin Google AI Studio Gemini Direct. S-a adăugat guardrail de plafon zilnic (`isDailyBudgetExceeded` / `DEFAULT_DAILY_BUDGET_CAP_USD` în `services/cost.ts`). |
