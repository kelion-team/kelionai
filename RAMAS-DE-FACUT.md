...
- [x] ORDIN 15 aug, P22 (verbatim): „se poate numi aplicatia Studioul de
      ...
      browserului la finalul fiecărui clip. + REȚETA VIDEO GRATUIT PRIN GOOGLE
      FLOW (ordin 17:23-17:32, verbatim: „prin google flow" + „fiecare user
      logat poate sa le foloseasca" + „exact la asta ma gindeam"): MĂSURAT
      (15 aug): Flow dă 50 credite gratuite/zi oricărui cont Google, fără
      abonament, dar N-ARE API — deci rețeta per user: Kelion scrie promptul
      șlefuit (buton Copiază + buton Deschide Flow) → userul generează pe
...
| K3 | „a rămas vreun PR nemerge-uit?" | ✅ | Am închis 17 PR-uri vechi, depășite (schelării de constructor iul, bug „litere sparte", docs de gazdă veche, soneria #110). Rămâne deschis doar #401 (ghid Google OAuth), decizia ta. |
...
| P13 | **Spațiul de încărcare în chat, mărit ȚINTIT** — MĂSURAT: /api/ingest stătea pe bodyLimit-ul global de 25MB (≈ fișier de 18MB după umflarea base64), oglindit cinstit în frontend (24MB). FIX: doar ruta de ingest urcă la 100MB (ca /api/chat, sub capul real Cloudflare); globalul de 25MB rămâne pentru restul rutelor; oglinda frontend → 96MB base64 (≈ fișier ~72MB), refuzul cinstit docTooLarge păstrat. Lacăte: spatiuIncarcare.test.ts + onestitateFrontend actualizat pe intenție. | 🔧 reparat în cod, de verificat live: un video de ~30-70MB se atașează în chat |
| P14 | **Proba buton-cu-buton a meniului Aplicații, pe viu** (ownerul, 15 aug: „trebuie sa iei fiecare buton din aplicatii si sa vezi ca functioneaza") — lanțurile din COD sunt încuiate (P5/lantGoogle), dar proba FUNCȚIONALĂ cap-la-cap cere sesiunea Google a ownerului (token-urile lui trăiesc în cookie). Plan: buton „Probează aplicațiile" (admin) care rulează cele 17 comenzi în sesiunea LUI, secvențial, și raportează pe fiecare: unealta chemată + ok/403/eroare — tabel de verdicte măsurate, nu bife pe cuvânt. Până atunci: descoperit și REPARAT clipping-ul care ascundea 🎬 (apps-menu fără max-height/overflow → coada listei cădea sub marginea ecranului). | ⏳ în coadă (clipping-ul reparat în PR-ul curent) |
| P15 | **„Iar aude bălării și scrie alte bălării în chat"** (ownerul, 15 aug, după P12) — MĂSURAT: audio-ul avea cadre-în-așteptare până la verdict, dar TEXTUL răspunsului curgea spre bandă cât verdictul era null (linia bloca doar `false`), deci replica la vorbire neadresată se SCRIA în chat chiar cu vocea suprimată; plus un AL DOILEA fail-open la finalul turei (fără nicio transcriere → cadrele se livrau). FIX: text-în-așteptare cu ACELAȘI verdict ca audio-ul (vărsat la toate cele 4 tranziții, golit la barge-in/tura gata) + fail-open-ul de la turaGata închis strict. Bălăriile AUZITE (transcriptul intrării) rămân vizibile pe bandă — diagnosticabilitate cerută pe 13 aug; ce nu se mai întâmplă e RĂSPUNSUL la ele. | 🔧 reparat în cod (PR curent), de verificat live: vorbire fără nume → nici voce, nici text de răspuns |
| P16 | **„Cheia Anthropic invalidă" era BULA DE CREDIT, nu cheia** (ownerul, 15 aug: „nu cheia e problema, e situația identică ca la bula de credit... vezi cum s-a rezolvat la credit și aplică și aici") — MĂSURAT, regula #2: (1) cheia se citea DOAR din process.env pe 3 nume fixe, ocolind mecanismul de aliasuri construit fix după lecția din 30 iul; (2) scara de modele era din 2024 (claude-3-5-sonnet etc., RETRASE) și nici nu conținea Fable 5 — orice cheie pica pe toată scara cu model_not_found, iar eșecul era pus pe seama cheii; (3) proba spunea „pune o cheie nouă" la ORICE status. FIX pe rețeta creditului: `anthropicKey` în config + ENV_ALIASES (7 nume acceptate, „ai scris X, citesc Y"); scara = familia Claude 5 cu `claude-fable-5` primul; verdict precis (doar 401 = cheie nerecunoscută; 403/429/5xx pe numele lor). Ownerul NU schimbă cheia — pe drept. | 🔧 reparat în cod (PR curent), de verificat live: Admin → token-checks / becul Fable trebuie să iasă verde cu cheia EXISTENTĂ |
| P17 | **Browserul mâinilor „nu funcționează" — testat cu lansare, nu declarat** (ownerul, 15 aug: „browserul acesta nu funcționează" + „ce-ar fi să testezi tot ce faci și lași funcțional") — MĂSURAT: deploy 4b instala Chromium cu TOATĂ ieșirea la /dev/null și declara „viu" pe codul de ieșire al INSTALĂRII — un browser cu biblioteci de sistem lipsă „reușea" instalarea și murea la lansare, tăcut, până se lovea omul de el. FIX: (1) deploy 4b — ieșirea instalării în /root/kelion/browser-install.log + verdict dintr-o LANSARE REALĂ de Chromium („VIU — probat" / „MORT + cauza"); (2) probaBrowserulMainilor() (browser.ts, cache 10 min) legată în system_health: becul „browserul mâinilor" cu eroarea reală, intră la probleme când e mort. | 🔧 reparat în cod (PR curent); prima re-publicare după merge arată verdictul lansării în logul de deploy |
