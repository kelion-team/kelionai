...
| B5 | **Cheia `sk_live` „K"** | ⚠️ **Necesită acțiune din partea ownerului (Stripe Dashboard)**: Aplicația KelionAI NU mai folosește Stripe pe nicio cale (plățile trec pe Revolut, cardul virtual a fost scos). Cheia `sk_live` "K" oferă acces total la contul Stripe și trebuie retrasă/revocată manual de owner din Stripe Dashboard -> Developers -> API Keys (un singur click „Revoke key"). Aceasta este o operațiune de securitate ce aparține exclusiv owner-ului de cont Stripe. |
