...
echo "== 6. Backup criptat s?pt?m?nal (cron duminic? 03:00) =="
...
( crontab -l 2>/dev/null | grep -v '/root/kelion/backup.sh' ; echo '0 3 * * 0 /root/kelion/backup.sh >> /root/kelion/backup.log 2>&1' ) | crontab -
