...
ruleaza_portile() {
  ...
  ( exec 9>&-; cd "$dir" && node --test scripts/verifica-butoane.test.mjs scripts/verifica-exporturi.test.mjs scripts/verifica-hardcodari.test.mjs scripts/verifica-creier-unic.test.mjs scripts/verifica-migrari.test.mjs scripts/inventar-audit.test.mjs scripts/verifica-contract-deploy.test.mjs scripts/release-train-preflight.test.mjs scripts/release-train-workflow.test.mjs ios/appstore-build.test.mjs deploy/lib/create-migration-proof.test.mjs deploy/lib/restore-verified-backup.test.mjs deploy/lib/caddy-security.test.mjs deploy/lib/codex-boundary.test.mjs deploy/lib/constructor-publication.test.mjs deploy/lib/network-config.test.mjs deploy/lib/compose-security.test.mjs deploy/lib/security-policy.test.mjs ) >/dev/null 2>&1 && R_UNIT=TRECE
