...
test('preflightul reutilizează autentificarea și se oprește înainte de orice mutație DB', () => {
  ...
  const authenticated = script.indexOf('node "$VALIDATOR"')
  const archiveList = script.indexOf('| pg_restore --list')
  ...
  const persistentDecrypt = script.indexOf('-out "$plaintext_dump"', preflightExit)
  const createScratch = script.indexOf("'CREATE DATABASE %I WITH TEMPLATE template0 OWNER %I'", persistentDecrypt)
  const preflightBody = script.slice(preflightExit, persistentDecrypt)
  ...
  assert.ok(archiveList > authenticated && databaseProbe > archiveList)
  assert.ok(diskProbe > databaseProbe && preflightExit > diskProbe)
  assert.ok(persistentDecrypt > preflightExit && createScratch > persistentDecrypt)
  ...
  assert.match(script, /archive_pipeline_status[\s\S]*restore_backup_decryption_failed[\s\S]*restore_dump_format_invalid/)
