...
test('workerul folosește direct OpenCode 1.18.25 cu Qwen local și acces complet prin sudo', () => {
  ...
  const launcher = worker.slice(worker.indexOf('export function openCodeExecArgs'), worker.indexOf('export function rootOpenCodeArgs'))
...
test('dovada permanentă leagă executorul, modelul și accesul complet de procesele reale', () => {
  const proof = read('.github/workflows/private-ai-constructor-proof.yml')
  const login = read('.github/workflows/vps-codex-login.yml')
  ...
  for (const workflow of [proof, login]) {
    assert.doesNotMatch(workflow, /\.constructor-finalized|final_receipt|schema=3/)
    ...
    assert.match(workflow, /awk -v target="\$model_file_path"[\s\S]*"\/proc\/\$llm_pid\/maps"/)
    assert.match(workflow, /FragmentPath --value[\s\S]*DropInPaths --value/)
  }
  ...
  assert.match(proof, /expected_llama_server_sha=\$\{20\}[\s\S]*expected_opencode_bin_sha=\$\{21\}/)
  assert.match(proof, /awk '\/\^Uid:\/ \{ print \$2 \}'[\s\S]*expected_opencode_argv[\s\S]*\/proc\/\$pid\/cwd/)
  assert.match(installer, /private-ai-web\.service\.d[\s\S]*90-kelion-constructor-full-access\.conf/)
  assert.match(upgrade, /private-ai-web\.service\.d[\s\S]*90-kelion-constructor-full-access\.conf/)
})
