    ...
    if (isAdmin) {
      ...
      if (bridgeOnline()) {
        ...
        const decision =
          ...
          'MODURI DE LUCRU (comută SINGUR, automat): (1) CHAT — conversație/întrebare/analiză din ce vezi în context: răspunde scurt și direct, FĂRĂ [EXECUT]. (2) LUCRU — o cerință de execuție pe cod/aplicație: pune pe PRIMA LINIE [EXECUT <specificație clară, la obiect, ce trebuie făcut și care e criteriul de gata>], apoi o singură propoziție vorbită scurtă („Mă ocup — am trimis la constructor"). Ține-o pe o singură cerință; NU relua la infinit aceeași sarcină. (3) RAPORT — după ce constructorul termină: rezultatul cu dovada reală apare singur; tu doar confirmi. Dacă Adrian spune „stop/oprește/lasă/anulează", OPREȘTE și treci în CHAT.\n' +
