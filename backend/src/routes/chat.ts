...
const ASK_BRAIN_TOOL: Tool = {
  name: 'ask_brain',
  description:
    'ESCALATION DOOR — use when a request needs the heavier conversation reasoning tier or a safe tool omitted from the lightweight turn. Product changes go only through build_software into build_jobs for the separate OpenCode (motor configurat separat) worker; this door never grants source, shell, repository, raw SQL, secret or deploy access.',
  input_schema: {
    type: 'object',
    properties: {
...
MĂSOARĂ, NU DECLARA (regula de fier a lui Adrian, 8 aug: „va trebui să folosească OBLIGATORIU toate testele și să măsoare orice răspuns"). Orice afirmație despre STAREA sistemului — merge / nu merge, cât durează, cât costă, câte sunt, e verde / e roșu — trebuie să vină dintr-o măsurătoare pe care ai făcut-o TU, în tura asta, cu o unealtă. Reguli, fără excepție:
  ...
  • CODUL ȘI PUBLICAREA NU SE EXECUTĂ ÎN PROCESUL WEB. Pentru o schimbare, folosește build_software o singură dată: ordinul validat intră direct în build_jobs, iar workerul separat OpenCode (motor configurat separat) rulează porțile și raportează factual starea.
  ...
  app.post<{
    ...
    if (esteAdminKelion(user.email)) {
      systemPrompt +=
        ...
        (turaCurata ? '' : `\n\nOWNER CHANGE FLOW: use build_software to persist a validated order directly in build_jobs for the separate OpenCode worker with its validated deployed engine. Follow the same jobId with constructor_status; only that worker may edit the worktree and only the master/deploy pipeline may publish after gates. Report commit and liveVersion only when measured. Never claim direct shell, repository, merge or deploy powers in this web process.`)
...
async function runTool(
  ...
  switch (block.name) {
    ...
    case 'build_software': {
      ...
      // apoi aceeași cerere intră direct în build_jobs pentru OpenCode (motor configurat separat).
    ...
    case 'constructor_status': {
      ...
      // worker OpenCode (motor configurat separat) + dispecerul backend + coada build_jobs — și raportează
      ...
      return JSON.stringify({
        constructor: config.constructorWorker.enabled
          ? 'OpenCode (motor configurat separat) activ — ordinele sunt revendicate exclusiv din build_jobs'
          : 'OpenCode (motor configurat separat) este dezactivat; ordinele rămân în build_jobs',
