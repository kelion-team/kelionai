...
// BOOTSTRAP: bind the port FIRST so Railway's healthcheck (/health → 200) passes
// within seconds, THEN initialise the DB in the background. The DB init used to
// run *before* listen — a slow or unreachable Postgres would push startup past
// the 120s healthcheck window, Railway declared the deploy unhealthy and rolled
// back (the 9 iul crash-loop). Now a DB hiccup can only delay chat persistence,
// never the boot itself; every route is already registered before we listen.
try {
...
// Runs AFTER listen, off the boot path, so it can never block the healthcheck.
setImmediate(() => {
  void (async () => {
    try {
      await initDb()
      await initAppFiles() // load installer masters (uploaded from Linux) into cache
      app.log.info('DB initialised')
    } catch (err) {
