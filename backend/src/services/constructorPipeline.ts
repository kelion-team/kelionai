        ...
        const same = current.task_id === taskId
          ...
          && (
            ...
            || (current.status === 'failed' && current.constructor_stage === 'failed'
              && current.publisher_last_error === 'stale_base')
          )
        return same
...
function publisherIncident(code: string): {
  ...
  if (code === 'stale_base') return {
    ...
    nextAction: 'Ciclul s-a oprit pe baza declarată; handoff-ul și dovezile se păstrează. Schimbarea bazei nu pornește automat o nouă execuție AI.',
  }
        ...
        if (code === 'stale_base') {
          await sql.query(
            `UPDATE constructor_pipeline SET publisher_lease_id=NULL,
                ...
                publisher_retry_not_before=NULL, updated_at=now()
              WHERE job_id=$1`,
            [jobId, code],
          )
          const terminal = await sql.query<PipelineRow>(
            `UPDATE build_jobs SET status='failed', constructor_stage='failed',
                progress='publisher_manual_restart_required', log=$2,
                retry_not_before=NULL, progress_at=now(), updated_at=now()
              WHERE id=$1
              RETURNING id AS job_id, status, constructor_stage, commit_sha, live_version`,
            [jobId, code],
          )
