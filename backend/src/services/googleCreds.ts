 ...
 *  by STT batch (asr.ts), STT streaming (asr-stream.ts) and TTS (tts.ts),
 *  which used to JSON.parse the same string in 5 spots. Returns null when the
 *  account is not configured OR the JSON is invalid: the callers then take
 *  their normal fallback (OpenAI) instead of throwing mid-request.
