...
/**
 * Detects known phantom hallucinations (e.g. "Greț.", "Subtitrare...", "Mulțumesc.")
 * generated by STT models (Chirp, Gemini, Whisper) from ambient silence or noise.
 */
export function isPhantomTranscript(text: string): boolean {
  ...
  // Known phantom hallucinations generated from background noise / silence
  const phantomExactOrPrefix = [
    'greț',
    'gret',
    'subtitrare',
  ...
  for (const p of phantomExactOrPrefix) {
    if (cleaned === p || cleaned.startsWith('subtitrar') || cleaned.startsWith('traducere')) {
      return true
    }
  }
...
/**
 * Calculates RMS (Root Mean Square) energy of 16-bit PCM audio samples.
 */
...
/**
 * Checks whether 16-bit PCM audio has minimum RMS speech energy.
 */
export function isSpeechAudio(buffer: Buffer, minRmsThreshold = 150): boolean {
  return calculatePcm16Rms(buffer) >= minRmsThreshold
}
...
async function transcribeFallbackGemini(audio: string, opts: TranscribeOpts): Promise<TranscribeResult> {
  ...
  const rawTranscript = r.transcript ?? ''
  const cleanTranscript = isPhantomTranscript(rawTranscript) ? '' : rawTranscript
  return { ok: true, lang: opts.langHint ?? null, transcript: cleanTranscript }
}
...
export async function transcribe(audioBase64: string, opts: TranscribeOpts = {}): Promise<TranscribeResult> {
  ...
  // If RAW PCM is provided, check audio energy threshold before network call
  if (opts.pcm) {
    const pcmBuf = Buffer.from(audio, 'base64')
    if (!isSpeechAudio(pcmBuf)) {
      return { ok: true, lang: opts.langHint ?? null, transcript: '' }
    }
  }
  ...
  if (config.useLocalVosk) {
    ...
    if (result.ok) {
      const clean = isPhantomTranscript(result.transcript) ? '' : result.transcript
      return { ok: true, lang: result.lang, transcript: clean }
    } else {
  ...
  try {
    ...
    const rawTranscript = r0?.alternatives?.[0]?.transcript ?? ''
    const cleanTranscript = isPhantomTranscript(rawTranscript) ? '' : rawTranscript
    return { ok: true, lang: r0?.languageCode ?? null, transcript: cleanTranscript }
  } catch {
    return transcribeFallbackGemini(audio, opts)
  }
}
