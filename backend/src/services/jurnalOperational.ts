...
export function curataTextJurnal(value: unknown, max = 500): string {
  ...
  return String(value ?? '')
    ...
    .replace(/\b(?:authorization|proxy-authorization)\s*:\s*[^\s,;]+(?:\s+[^\s,;]+)?/gi, 'authorization=[redacted]')
    .replace(/\b(bearer|token|api[_ -]?key|password|secret)\s*[:=]\s*\S+/gi, '$1=[redacted]')
    .replace(/\b(cvc|cvv|security[_ -]?code)\s*[:=]\s*\d{3,4}\b/gi, '$1=[redacted]')
    .replace(/\b(?:\d[ -]*?){13,19}\b/g, '[redacted-pan]')
    .replace(/\bsk-(?:proj|admin)-[A-Za-z0-9_-]+\b/g, '[redacted-openai-key]')
