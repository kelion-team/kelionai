...
describe('Constructor Codex-only preflight', () => {
  ...
  it('accepts concrete repository work and selects only the separate worker', () => {
    ...
    expect(result.aiRecomandat).toBe('codex_worker')
    expect(result.capacitatiNecesare).toEqual(expect.arrayContaining(['cod', 'repo', 'backend', 'teste']))
    expect(result.clasament.map((entry) => entry.cheie)).toEqual(['codex_worker'])
    expect(AI_CONSTRUCTORI.map((entry) => entry.cheie)).toEqual(['codex_worker'])
  })
