...
if (!engine) {
  ...
  const r = await api(token, loc, `${col}/engines?engineId=kelion-agenti`, {
    method: 'POST',
    body: JSON.stringify({ displayName: 'Kelion — agenți', solutionType: 'SOLUTION_TYPE_CHAT', industryVertical: 'INDUSTRY_VERTICAL_GENERIC', chatEngineConfig: { agentCreationConfig: { business: 'Kelion AI — asistent live', defaultLanguageCode: 'ro', timeZone: 'Europe/London' } } }),
  })
