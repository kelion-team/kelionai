...
import { androidManifestConfig, appleAssociation, assetLinks, product, tauriConfig } from './genereaza-config-platforme.mjs'
...
const executabile = tracked.filter((path) => ['.aab', '.apk', '.appx', '.exe', '.ipa', '.msix', '.msixbundle'].includes(extname(path).toLowerCase()))
