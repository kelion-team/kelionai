...
async function build(order) {
  ...
  const prompt =
    ...
    `- Task (SWARM de sub-agenti PARALELI — capabilitatea K2 „Agent Swarm", Adrian 13 iul: „sa foloseasca tot din Kimi 2.7"): la o cerinta MARE sau cu multe parti INDEPENDENTE, DESCOMPUNE-O agresiv si lanseaza MULTI sub-agenti in acelasi timp (nu unul cate unul) — ex. audit pe 8 module = 8 Task-uri deodata; cautare aceeasi problema in N fisiere = N Task-uri; explorare din mai multe unghiuri = cate un Task pe unghi. Le pornesti PE TOATE intr-un singur mesaj (mai multe blocuri Task) ca sa ruleze concomitent, apoi aduni si sintetizezi rezultatele. Asta e modul de a merge RAPID pe munca mare. LIMITE: NU pentru pasi care depind secvential (localizezi -> editezi -> verifici acelasi fisier — acolo lucrezi tu direct), si NU pentru o schimbare simpla intr-un singur fisier (e balast). Regula: daca poti imparti munca in bucati care nu se calca, imparte-o si ruleaza-le in paralel.\n` +
...
async function deployApproved(r) {
  ...
  if (pubRes.code !== 0) {
    ...
    // GARD ANTI-PUBLICARE CU TOKEN GITHUB INVALID (Adrian, 14 iul): token invalid =
    // problemă GLOBALĂ (nu doar a acestui release). Dacă publicarea a eșuat cu
    // 401/Bad credentials, oprim TOT constructorul, blocăm pending release-urile și
    // anunțăm pe monitor — nu mai încercăm niciun deploy până la înlocuirea cheii.
    ...
    // Altă cauză (tranzitorie sau specifică ACESTUI release): întrerupătorul
    // universal — un eșec poate fi tranzitoriu (redeploy în curs); DOUĂ la rând =
    // problemă reală → tripează pe acest release (îl marchează `failed` + anunță
    // adminul o dată), fără să oprească tot constructorul. Zero retry orb.
